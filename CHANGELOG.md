...
- **LLM-powered session summaries** (#5). New migration 004 adds a `session_summaries` table. `src/core/analyze.ts` builds a token-budgeted prompt from session metadata and user messages, and an injected `LLMClient` generates a structured summary (one-liner, what-tried, outcome, notable events, blog hooks, tags). `AnthropicClient` is the default implementation (Haiku 4.5 by default). New `csk analyze` CLI command with `--dry-run`, `--limit`, `--project`, `--since`, `--model`. New MCP tools `csk_summarize` (on-demand generation + cache) and `csk_recap` (per-project recent-summary digest).
